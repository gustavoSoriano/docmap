...
export const tasksHandler =
  (kv: Deno.Kv) => async (req: Request, url: URL): Promise<Response> => {
    const segments = url.pathname.replace(/^\/tasks\/?/, '').split('/').filter(
      Boolean,
    );
    const id = segments[0];
    ...
    if (req.method === 'GET' && id) {
      ...
      return json({
        ...task,
        deepLink: `http://127.0.0.1:3333/#task/${task.id}`,
      });
    }
    ...
    if (req.method === 'PUT' && id === 'reorder') {
      ...
      if (!input.status || !Array.isArray(input.ids)) {
        return badRequest('status and ids required');
      }
    ...
    if (req.method === 'PUT' && id) {
      ...
      return json({
        ...updated,
        deepLink: `http://127.0.0.1:3333/#task/${updated.id}`,
      });
    }

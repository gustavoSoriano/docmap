...
async (
  ...
  if (req.method === 'POST' && url.pathname === '/system/restore-pick') {
    ...
    try {
      const count = await importKv(
        kv,
        body as Parameters<typeof importKv>[1],
        replace,
      );
      return json({ ok: true, imported: count, path });
    } catch (err) {
      return badRequest(
        err instanceof Error ? err.message : 'restore falhou',
      );
    }
  }
  ...
  if (req.method === 'POST' && url.pathname === '/system/open-url') {
    ...
    try {
      ...
      return badRequest(
        err instanceof Error ? err.message : 'falha ao abrir URL',
      );
    }
  }

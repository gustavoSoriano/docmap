import type { Skill, SkillPreview, CreateSkillInput, UpdateSkillInput } from './types.ts';
...
export const createSkill = async (kv: Deno.Kv, input: CreateSkillInput): Promise<Skill> => {
  const skill: Skill = {
...
export const getSkillById = async (kv: Deno.Kv, id: string): Promise<Skill | null> => {
  const entry = await kv.get<Skill>(key(id));
  return entry.value;
};

// resolve por ID ou por name (slug)
export const resolveSkill = async (kv: Deno.Kv, idOrName: string): Promise<Skill | null> => {
...
export const updateSkill = async (
  kv: Deno.Kv, id: string, input: UpdateSkillInput,
): Promise<Skill | null> => {
  const existing = await getSkillById(kv, id);
  if (!existing) return null;
  const updated: Skill = {
    ...existing,
    ...(input.name        !== undefined ? { name:        toSlug(input.name) || existing.name } : {}),
...
export const deleteSkill = async (kv: Deno.Kv, id: string): Promise<boolean> => {

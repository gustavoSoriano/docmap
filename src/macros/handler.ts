import { createMacro, getMacroById, updateMacro, deleteMacro, listMacros } from './store.ts';
...
export const macrosHandler = (kv: Deno.Kv, workspace: WorkspaceRef) =>
  async (req: Request, url: URL): Promise<Response> => {
    const parts = url.pathname.replace(/^\/macros\/?/, '').split('/').filter(Boolean);
    const id    = parts[0];
    ...
    if (req.method === 'POST' && id && action === 'run') {
      ...
      if (result.blocked) return json({ error: 'blocked', reason: result.reason }, 403);
    ...
    if (req.method === 'DELETE' && id) return json({ ok: await deleteMacro(kv, id) });
...
const postMacro = async (kv: Deno.Kv, req: Request): Promise<Response> => {
  ...
  if (!input.title || !input.script) return badRequest('title and script required');
  return json(await createMacro(kv, input), 201);
};

const putMacro = async (kv: Deno.Kv, id: string, req: Request): Promise<Response> => {
  let body: unknown;
  try { body = await req.json(); } catch { return badRequest('Invalid JSON'); }

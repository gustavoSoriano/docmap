import type { Macro, MacroPreview, MacroInterpreter, CreateMacroInput, UpdateMacroInput } from './types.ts';
...
const toPreview = (m: Macro): MacroPreview => ({
  ...
  description: m.description, interpreter: m.interpreter,
  createdAt: m.createdAt, updatedAt: m.updatedAt,
});

export const createMacro = async (kv: Deno.Kv, input: CreateMacroInput): Promise<Macro> => {
  const macro: Macro = {
...
export const getMacroById = async (kv: Deno.Kv, id: string): Promise<Macro | null> => {
  const entry = await kv.get<Macro>(key(id));
  return entry.value;
};

export const updateMacro = async (
  kv: Deno.Kv, id: string, input: UpdateMacroInput,
): Promise<Macro | null> => {
  ...
  const updated: Macro = {
    ...
    ...(input.script      !== undefined ? { script,      interpreter: detectInterpreter(script) } : {}),
    updatedAt: new Date().toISOString(),
  };
...
export const deleteMacro = async (kv: Deno.Kv, id: string): Promise<boolean> => {
